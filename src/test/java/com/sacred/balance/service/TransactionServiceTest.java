...
class TransactionServiceTest {
    ...
    @Test
    void shouldHandleConcurrentTransactionsOnSameAccount() throws InterruptedException {
        // 创建账户
        Account account1 = new Account("A005", 1000.0);
        Account account2 = new Account("A006", 500.0);
        Account account3 = new Account("A007", 300.0);
        ...
        tx1.setTransactionId("T003");
        tx1.setSourceAccount("A005");
        tx1.setDestinationAccount("A006");
        tx1.setAmount(100.0);

        Transaction tx2 = new Transaction();
        tx2.setTransactionId("T004");
        tx2.setSourceAccount("A005");
        tx2.setDestinationAccount("A007");
        tx2.setAmount(200.0);

        Transaction tx3 = new Transaction();
        tx3.setTransactionId("T005");
        tx3.setSourceAccount("A006");
        tx3.setDestinationAccount("A005");
        tx3.setAmount(50.0);
        ...
        CompletableFuture<TransactionResult> future1 = CompletableFuture.supplyAsync(() -> {
            try {
                return transactionService.processTransaction(tx1);
            } finally {
                latch.countDown();
            }
        }, executor);

        CompletableFuture<TransactionResult> future2 = CompletableFuture.supplyAsync(() -> {
            try {
                return transactionService.processTransaction(tx2);
            } finally {
                latch.countDown();
            }
        }, executor);

        CompletableFuture<TransactionResult> future3 = CompletableFuture.supplyAsync(() -> {
            try {
                return transactionService.processTransaction(tx3);
            } finally {
                latch.countDown();
            }
        }, executor);
        ...
        // 获取结果
        TransactionResult result1 = future1.join();
        TransactionResult result2 = future2.join();
        TransactionResult result3 = future3.join();
        ...
        // 验证最终余额
        Optional<Account> finalAccount1 = accountRepository.findByAccountNumber("A005");
        Optional<Account> finalAccount2 = accountRepository.findByAccountNumber("A006");
        Optional<Account> finalAccount3 = accountRepository.findByAccountNumber("A007");

        assertTrue(finalAccount1.isPresent());
        assertTrue(finalAccount2.isPresent());
        assertTrue(finalAccount3.isPresent());

        // A005: 1000 - 100 - 200 + 50 = 750
        // A006: 500 + 100 - 50 = 550
        // A007: 300 + 200 = 500
        assertEquals(750.0, finalAccount1.get().getBalance());
        assertEquals(550.0, finalAccount2.get().getBalance());
        assertEquals(500.0, finalAccount3.get().getBalance());
    }
